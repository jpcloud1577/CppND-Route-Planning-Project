...
bool RoutePlanner::fValue(const RouteModel::Node* n, const RouteModel::Node*  m){
    float f1 = n->g_value + n->h_value;
    float f2 = m->g_value + m->h_value;
    return f1>f2;
}

RouteModel::Node *RoutePlanner::NextNode() {
    for(auto n: open_list){
        sort(open_list.begin(), open_list.end(), RoutePlanner::fValue);
    }
    
    RouteModel::Node* LowSum = this->open_list.back();
...
void RoutePlanner::AStarSearch() {
    ...
    while (!open_list.empty()){
        ...
        float dist = current_node->distance(*this->end_node);
